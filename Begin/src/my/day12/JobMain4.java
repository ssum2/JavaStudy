...
//				gujikjaArr[deleteIndex+i] = gujikjaArr[deleteIndex+i+1]; // 지워질 방에 있는 배열 바로 뒤(+1)의 값을 지워질 방에 넣어줌
//				gujikjaArr[deleteIndex+i+1] = null; // 뒤에서 옮긴 방은 null로 초기화
//			}
//		}
//		강사님이 오류 수정해준 것 -> j라는 변수 하나 설정해서 deleteIndex를 넣어주고, j에 증감식, 진짜 deleteIndex는 변하지 않도록 함		
		else {
			for(int i=0, j=deleteIndex; i<(Gujikja.count-1-deleteIndex); i++, j++) { 
				gujikjaArr[j] = gujikjaArr[j+1]; 
				gujikjaArr[j+1] = null; 
			}
		}
