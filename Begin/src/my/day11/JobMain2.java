...
//							break;
	...
	// JobMain3이랑 다른 점 : 관리자 로그인 기능을 메소드로 따로 빼서 씀		
			case "5": // 5. 로그인(관리자)
				boolean adminLogin = jobapp.loginAdmin(sc);
				int isBreak = 0;
				if(adminLogin) {
					do {
						JobMain2.adminMenu();
						String adminMenuNo = sc.nextLine();
						
						switch (adminMenuNo) {
						case "1": // 1. 모든 구직자 정보 조회
							for(int i=0; i<Gujikja.count; i++) {
								jobapp.gujikjaArr[i].getGujikjaInfo();
							}
							break;

							
						case "2": // 2. 모든 구인회사 정보 조회
							break;
						case "3": // 3. 특정 구직자 삭제
							break;
						case "4": // 4. 특정 구인회사 삭제
